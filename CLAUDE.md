...
3. Add a changelog entry at the top of `docs/changelog.mdx` (`## Jul 17, 2026` format).
...
**Content Collections** (defined in `src/content.config.ts`, glob-loaded from repo root):
- `docs/*.mdx` - Documentation pages (install, mirrors, changelog, contact), rendered by `src/pages/[slug].astro` through `DocsLayout`
...
- `src/pages/` - index (homepage), intro, features, `[slug]` (docs), `blog/` + `tags/` (lists, pagination, post detail with ShareButtons + JSON-LD), 404, feeds (`rss.xml.ts`, `atom.xml.ts` via `src/lib/feed.ts`), `robots.txt.ts`
